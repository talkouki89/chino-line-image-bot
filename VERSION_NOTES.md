...
- 修正 Lolicon 抽圖結果的 `aiType` 顯示邏輯；現在會依 API 回傳值顯示 `否` / `未知` / `是`，避免把 `aiType=1` 的未知狀態誤判成 AI。
- 抽圖指令改為大小寫不敏感，`隨機無AI`、`r18無AI` 等大小寫混用都能正常觸發。
- 媒體下載與回覆下載指令改為大小寫不敏感，支援 `YT:`、`FB:`、`IG:`、`PH:`、`TK:`、`X:` 與 `回覆搜X` 等寫法。
- 管理工具 `ren`、`speedtest`、`mid:`、`Contact @人` 也改為大小寫不敏感，方便在不同輸入法或自動更正情境下使用。
- README 補上相關指令大小寫不敏感，以及抽圖 AI 判斷三態說明。
